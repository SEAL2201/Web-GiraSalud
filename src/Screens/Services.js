...
import exampleImage from '../assets/example.jpg'; // Ejemplo de imagen de servicio
  ...
  return (
    <div name="Servicios" className="bg-white py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-5xl font-extrabold text-gray-900 text-center">SERVICIOS</h2>
        <p className="mt-4 text-3xl text-gray-600 text-center">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Disclosure key={index} as="div" className="bg-white shadow-md rounded-lg p-6">
              {({ open }) => (
                <>
                  <Disclosure.Button className="flex justify-between w-full text-left text-3xl font-medium text-gray-900 focus:outline-none">
                  ...
                  <Disclosure.Panel className="pt-4 text-lg text-gray-600">
                    <p className="text-3xl">{service.description}</p>
                    <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {service.details.map((detail, index) => (
                        <div key={index} className="flex flex-col items-center">
                          ...
                          <p className="text-3xl font-medium">{detail.title}</p>
                        </div>
                      ))}
                    </div>
                  </Disclosure.Panel>
